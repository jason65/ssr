<template>
  <div>
    <kindo-box icon="el-icon-search" title="条件筛选">
      <el-form inline label-position="right" label-width="100px" :model="form" @submit.native.prevent @keyup.enter.native="get">
        <el-form-item label="用户名：">
          <el-input v-model.trim.trim="form.loginNo"></el-input>
        </el-form-item>
        <el-form-item label="用户工号：">
          <el-input v-model.trim.trim="form.emplNo"></el-input>
        </el-form-item>
      </el-form>
    ...
    <kindo-box icon="el-icon-document" title="用户列表">
      <kindo-table ref="table" :url="this.api.get" :queryParam="form" :height="null">
        ...
        <el-table-column label="用户名" prop="loginNo" width="120" align="left" sortable='custom' show-overflow-tooltip></el-table-column>
        <el-table-column label="用户工号" prop="emplNo" width="120" align="left" sortable='custom' show-overflow-tooltip></el-table-column>
        <el-table-column label="用户昵称" prop="emplName" width="120" align="left" sortable='custom' show-overflow-tooltip></el-table-column>
        <el-table-column label="用户状态" prop="status" min-width="120" align="left" sortable='custom'>
          <template slot-scope="scope">
        ...
        <el-table-column label="操作" width="100" align="center" fixed="right">
          <template slot-scope="scope">
            ...
            <el-button title="删除" type="text" icon="el-icon-delete" v-disabled="'deleteData'" @click="del(scope.$index, scope.row)"></el-button>
          </template>
        </el-table-column>
      </kindo-table>

      <div slot="control">
        <el-button icon="el-icon-plus" v-disabled="'add'" @click="insert">新增</el-button>
      </div>
    </kindo-box>
          ...
          <el-row>
            <el-col :span="11">
              ...
              <el-form-item label="用户状态：" prop="status">
                <el-select v-model.trim="model.status" placeholder="请选择状态" style="width:100px">
              ...
              <el-form-item label="性别：" prop="sex">
                <el-select v-model.trim="model.sex" placeholder="" clearable>
            ...
            <el-col :span="11" :offset="2">
              <el-tree style="height: 350px;" ref="orgTree" highlight-current :props="orgProp" :data="orgData" node-key="id" @current-change="nodeChange"></el-tree>
            </el-col>
          </el-row>
        </el-form>

        <el-form v-show="active === 1">
          <el-transfer style="height: 340px;" v-model.trim="menuRoles" filterable :titles="['可选角色', '拥有角色']" :button-texts="['', '']" :data="source.menuRoles" :props="{ key: 'id', label: 'roleName' }">
          </el-transfer>
        </el-form>

        <el-form v-show="active === 2">
          <el-transfer style="height: 340px;" v-model.trim="dataRoles" filterable :titles="['可选角色', '拥有角色']" :button-texts="['', '']" :data="source.dataRoles" :props="{ key: 'id', label: 'roleName' }">
          </el-transfer>
        </el-form>
      </div>
  ...
  data() {
    return {
      ...
      rules: {
        ...
        age: [{ pattern: kindo.validate.pattern.pInteger, message: '年龄必须为正整数', trigger: 'change' }],
        status: [{ required: true, message: '请选择用户状态', trigger: 'change' }],
        name: [{ min: 0, max: 16, message: '请输入最多16个字', trigger: 'change' }],
        idCard: [{ pattern: kindo.validate.pattern.idCard, message: '请输入正确身份证号码格式', trigger: 'change' }],
        phone: [{ pattern: kindo.validate.pattern.telephone, message: '请输入正确电话号码格式', trigger: 'change' }]
      },
      menuRoles: [],
      dataRoles: []
    }
  ...
  methods: {
    ...
    next() {
      if (this.active === 0) {
          ...
          if (valid) {
            if (this.model.id) {
              ...
              this.$http.post(this.api.insert, this.model).then(res => {
                ...
                this.getUserRole(this.model.id).then(res => {
                  this.active = this.active + 1
                })
              })
            }
        ...
        this.$http.post(this.api.updateMenuRole, param).then(res => {
          this.active = this.active + 1
        })
      } else if (this.active === 2) {
        // 保存用户数据角色信息
        let param = { userId: this.model.id, dataRoleIds: this.dataRoles.toString() }
        this.$http.post(this.api.updateDataRole, param).then(res => {
          this.active = this.active + 1
        })
    ...
    getUserRole(userId) {
      return this.$http
        .all([this.$http.get(this.api.getMenuRoleById, { params: { userId } }), this.$http.get(this.api.getDataRoleById, { params: { userId } })])
        .then(
