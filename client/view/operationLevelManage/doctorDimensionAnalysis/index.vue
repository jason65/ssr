<template>
  <div>
    <kindo-box title="条件筛选" icon="el-icon-search">
      <el-form onsubmit="return false;" :model="search" ref="searchForm" :inline="true" label-width="100px" @keyup.enter.native="get">
        ...
        <el-form-item label="医生：">
          <el-select v-model="search.ysValue" size="mini" @blur="(ev)=>{blurSel(ev,'search','ysValue','doctorList')}" placeholder="请输入选择" clearable filterable :loading="loading" remote :remote-method="(query) => remoteMethod(query, 'loading', 'doctorList')">
            <el-option v-for="item in list.doctorList" :key="item.value" :label="item.label" :value="item.value">
            </el-option>
          </el-select>
        </el-form-item>
        <!-- <el-form-item label="角色类型："> -->
        <!-- <el-select v-model.trim="search.ysType" placeholder="" filterable style="width:180px;"> -->
        <!-- <el-option v-for="(item,index) in list.roleList" :key="index" :label="item.label" :value="item.value"> </el-option> -->
        <!-- </el-select> -->
        <!-- </el-form-item> -->
      </el-form>
      <el-button icon="el-icon-search" type="primary" slot="control" @click="query">查询</el-button>
    </kindo-box>
    ...
    <kindo-box title="手术分布详情" icon="fa fa-list">
      <kindo-table :url="config.api.tablelist" :queryParam="search" ref="tableListRef" :default-sort="{prop:'sscs',order:'ascending'}">
        <el-table-column prop="doctorName" label="医生姓名" width="110px" header-align="center" :show-overflow-tooltip="true">
          <template slot-scope="scope">
            {{scope.row.doctorName===null?'-':scope.row.doctorName}}
          </template>
        </el-table-column>
        <el-table-column prop="deptName" label="出院科室" min-width="140px" header-align="center" :show-overflow-tooltip="true">
          <template slot-scope="scope">
        ...
        <el-table-column prop="sscs" label="手术病例总数" width="130px" header-align="center" align="right" sortable='custom' :show-overflow-tooltip="true">
          <template slot-scope="scope">
            <el-button type="text" @click="goSubstandardPage(scope,null,scope.row.doctorName) ">{{scope.row.sscs===null?'-':scope.row.sscs}}</el-button>
          </template>
        </el-table-column>
        <el-table-column prop="cyrs" label="出院人数" width="120px" header-align="center" align="right" sortable='custom' :show-overflow-tooltip="true"></el-table-column>
        <el-table-column prop="sscs1" label="一级" width="80px" header-align="center" align="right" sortable='custom' :show-overflow-tooltip="true">
          <template slot-scope="scope">
            <el-button type="text" @click="goSubstandardPage(scope,'A',scope.row.doctorName) ">{{scope.row.sscs1===null?'-':scope.row.sscs1}}</el-button>
          </template>
        </el-table-column>
        <el-table-column prop="sscs2" label="二级" width="80px" header-align="center" align="right" sortable='custom' :show-overflow-tooltip="true">
          <template slot-scope="scope">
            <el-button type="text" @click="goSubstandardPage(scope,'B',scope.row.doctorName) ">{{scope.row.sscs2===null?'-':scope.row.sscs2}}</el-button>
          </template>
        </el-table-column>
        <el-table-column prop="sscs3" label="三级" width="80px" header-align="center" align="right" sortable='custom' :show-overflow-tooltip="true">
          <template slot-scope="scope">
            <el-button type="text" @click="goSubstandardPage(scope,'C',scope.row.doctorName) ">{{scope.row.sscs3===null?'-':scope.row.sscs3}}</el-button>
          </template>
        </el-table-column>
        <el-table-column prop="sscs4" label="四级" width="80px" header-align="center" align="right" sortable='custom' :show-overflow-tooltip="true">
          <template slot-scope="scope">
            <el-button type="text" @click="goSubstandardPage(scope,'D',scope.row.doctorName) ">{{scope.row.sscs4===null?'-':scope.row.sscs4}}</el-button>
          </template>
        </el-table-column>
        <el-table-column prop="sscstot" label="三四级手术数量" width="160px" header-align="center" sortable='custom' align="right" :show-overflow-tooltip="true">
          <template slot-scope="scope">
            <el-button type="text" @click="goSubstandardPage(scope,'E',scope.row.doctorName)">{{scope.row.sscstot===null?'-':scope.row.sscstot}}</el-button>
          </template>
        </el-table-column>
        <el-table-column prop="sszb" label="三四级手术占比(%)" width="170px" header-align="center" sortable='custom' align="right" :show-overflow-tooltip="true">
          <template slot-scope="scope">
            {{scope.row.sszb===null?'-':scope.row.sszb}}
          </template>
        </el-table-column>
        <el-table-column prop="zhjf" label="综合积分" width="120px" header-align="center" sortable='custom' align="right" :show-overflow-tooltip="true">
          <template slot-scope="scope">
...
export default {
  ...
  data() {
    return {
      ...
      loading: false,
      list: {
        roleList: [{ label: '手术医生', value: '1' }, { label: '医助', value: '2' }],
        doctorList: []
      },
      stackbar: {
        title: {
  ...
  created() {
    if (JSON.stringify(this.$route.query) !== '{}') {
      ...
      if (this.$route.query.ysValue && !kindo.validate.isEmpty(this.$route.query.ysValue)) {
        this.search.ysValue = this.$route.query.ysValue
      }
  ...
  methods: {
    ...
    queryBarChart() {
      ...
      this.$http.get(config.api.chart, { params: params }).then(res => {
        ...
        if (res.data && res.data.length > 0) {
          ...
          // 这里做所选科室排列第一
          if (this.search.ysValue !== '') {
            let indexObj = { index: 0, obj: {} }
            sourceArr.map((item, index) => {
              if (item.doctorName === this.search.ysValue) {
            ...
            if (JSON.stringify(indexObj.obj) !== '{}') {
              ...
              sourceArr.forEach((o, i) => {
                xAsisArr.push(o.doctorName)
                tempArr1.push({ name: o.doctorCode, value: o.sscs1 })
                tempArr2.push({ name: o.doctorCode, value: o.sscs2 })
                tempArr3.push({ name: o.doctorCode, value: o.sscs3 })
                tempArr4.push({ name: o.doctorCode, value: o.sscs4 })
                tempArr5.push({ name: o.doctorCode, value: o.cyrs })
              })
            }
          } else {
            sourceArr.forEach((o, i) => {
    ...
    goSubstandardPage(scope, level) {
      ...
      if (!kindo.validate.isEmpty(scope.row.doctorName)) {
        params.sz1 = scope.row.doctorName
      }
      this.$router.push({
      ...
      kindo.cache.set('goBack', { from: '/6945d60d-9868-4d76-a4b0-f1d3a5a63423/49614502-8ae6-4326-815b-2412b4286b92', params: Object.assign({}, this.search, this.dateComb) }, 'session')
    },
    exportTable() {
      this.setSearchDate()
