<template>
  <div>
    <kindo-box title="条件筛选" icon="fa-search">
      ...
      <div slot="control">
        <el-button type="primary" icon="el-icon-search" v-disabled="'search'" @click="getData">查询</el-button>
      </div>
    </kindo-box>

    <kindo-box title="数据角色" icon="fa-area-chart">
      <kindo-table :url="url" :queryParam="form" ref="table">
        ...
        <el-table-column label="操作" width="100" align="center" fixed="right">
          <template slot-scope="scope">
            <el-button title="编辑" type="text" icon="el-icon-edit" v-disabled="'edit'" @click="editUser(scope.$index, scope.row)"></el-button>
            <el-button title="删除" type="text" icon="el-icon-delete" v-disabled="'deleteData'" @click="deleteUser(scope.$index, scope.row)"></el-button>
          </template>
        </el-table-column>
      </kindo-table>

      <div slot="control">
        <el-button icon="el-icon-plus" v-disabled="'add'" @click="openAddModel">新增</el-button>
      </div>
    </kindo-box>
    ...
    <el-dialog v-drag width="1000px" :title="'权限'+(model.id===''?'新增':'编辑')" @close="model.active = 0" :visible.sync="dialogVisible">
      ...
      <div v-show="model.active === 0">
        <el-form :model="model" :rules="rules" ref="model" label-width="120px">
...
export default {
  data() {
    return {
      ...
      rules: {
        roleName: [
          { required: true, message: '请输入数据角色名称', trigger: 'change' },
  ...
  methods: {
    ...
    editUser(index, row) {
      kindo.util
        ...
        .then(() => {
          this.$http
            .all([
              this.$http.get(config.api.getRoleUser, { params: { id: row.id } }),
              this.$http.get(config.api.getDataRole, { params: { roleId: row.id } })
            ])
            .then(
              this.$http.spread((acct, perms) => {
                let param = {}
    ...
    save() {
      ...
      this.$http.post(config.api.updateDataRole, params).then(res => {
