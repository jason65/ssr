<template>
  <div>
    ...
    <kindo-box title="菜单角色" icon="fa-area-chart">
      <kindo-table :url="url" :queryParam="form" ref="table">
        ...
        <el-table-column label="操作" width="100" align="center" fixed="right">
          <template slot-scope="scope">
            <el-button title="编辑" type="text" icon="el-icon-edit" @click="editUser(scope.$index, scope.row)"></el-button>
...
export default {
  data() {
    return {
      ...
      rules: {
        roleName: [
          { required: true, message: '请输入菜单角色名称', trigger: 'change' },
      ...
      this.$nextTick(function() {
        ...
        this.$http
          .all([
            ...
            this.$http.get(config.api.getMenuRole, { params: { roleId: row.id } })
          ])
          .then(
            this.$http.spread((acct, perms) => {
              let param = {}
    ...
    checkALL() {
      if (this.checked) {
        for (let k in this.source.buttonAuth) {
          this.source.buttonTypes.forEach((v, i, arr) => {
