  ...
  <div>
    <el-row>
      <el-col :span="11">
        <el-form inline :model="hosForm" :rules="rules" ref="hosModel" onsubmit="return false;" @keyup.enter.native="getHos">
          <el-form-item prop="name">
            <el-tooltip effect="dark" content="请输入医院诊断编码或名称" placement="top">
              <el-input v-model="hosForm.name" placeholder="医院诊断..." style="width:185px;"></el-input>
            </el-tooltip>
          </el-form-item>

          <el-form-item>
            <el-button icon="search" @click="getHos">查 询</el-button>
            <el-button icon="upload" v-disabled="'save'" @click="() => { this.uploadVisible = true }">上传医院编码</el-button>
        ...
        <kindo-table :url="hosApi" ref="hos" :queryParam="hosForm" :selection-change="hosOnSelect">
          ...
          <el-table-column prop="code" label="医院诊断编码" width="140" sortable="custom" show-overflow-tooltip>
            <template slot-scope="scope">
              <span>{{ scope.row.code }}</span>
              <span v-if="scope.row.appendCode" style="color: firebrick">( {{ scope.row.appendCode }} )</span>
            </template>
          </el-table-column>
          <el-table-column prop="name" label="医院诊断名称" min-width="140" sortable="custom" show-overflow-tooltip> </el-table-column>
          <el-table-column prop="states" label="状态" width="100" align="center" sortable="custom" show-overflow-tooltip>
            <template slot-scope="scope">
      ...
      <el-col :span="2" style="text-align:center;">
        <el-button icon="share" @click="submitForRecommend">确 认 此 关 联 </el-button>
      </el-col>
      <el-col :span="11">
        <el-form inline :model="standardForm" :rules="rules" ref="standardModel" onsubmit="return false;" @keyup.enter.native="getStandard">
          <el-form-item prop="name" style="margin-left:50px;">
            <el-tooltip effect="dark" content="请输入标准诊断编码或名称" placement="top">
              <el-input v-model="standardForm.name" placeholder="标准诊断..." style="width:185px;"></el-input>
            </el-tooltip>
          </el-form-item>
          <el-form-item>
        ...
        <kindo-table :url="standardApi" ref="standard" :queryParam="standardCondition" :selection-change="standardOnSelect">
          <el-table-column type="selection" width="28"></el-table-column>
          <el-table-column prop="code" label="标准诊断编码" width="145" sortable="custom" show-overflow-tooltip>
            <template slot-scope="scope">
              <span>{{ scope.row.code }}</span>
              <span v-if="scope.row.appendCode" style="color: firebrick">( {{ scope.row.appendCode }} )</span>
            </template>
          </el-table-column>
          <el-table-column prop="name" label="标准诊断名称" min-width="145" sortable="custom" show-overflow-tooltip inline-template>
            <span v-html="row.name"></span>
          </el-table-column>
        </kindo-table>
      </el-col>
    </el-row>
    <el-dialog top="5%" v-drag size="tiny" v-model="uploadVisible" title="术语对接导入">
      <el-form>
        <el-form-item>
          <el-alert title="上传过程中系统将锁定, 请耐心等待完成..." type="info" :closable="false"> </el-alert>
        </el-form-item>
        <el-form-item>
          <el-upload ref="upload" drag accept=".xlsx,.xls " :headers="uploadHeaders" :on-success="uploadSuccess" :before-upload="beforeAvatarUpload" :action="uploadApi">
            ...
            <div class="el-upload__tip" slot="tip">只能上传xlsx/xls 文件，且不超过20000kb</div>
          </el-upload>
        </el-form-item>
      </el-form>
    </el-dialog>
...
export default {
  data() {
    return {
      ...
      rules: {
        name: [{ min: 0, max: 64, message: '请最多输入64个数字字母或汉字', trigger: 'blur' }]
      }
    }
  },
  ...
  methods: {
    ...
    // 删除医院诊断
    deleteHos(row) {
      this.$http.get(config.api.deleteHos, { params: { id: row.id } }).then(() => {
        kindo.util.alert('删除医院诊断成功')
        this.getHos()
        this.getStandard()
      })
    },
    ...
    submitForRecommend() {
      if (this.hosSelection.length === 0) {
        kindo.util.alert('请至少选择一条医院诊断', undefined, 'warning')
        return
      }

      if (this.standardSelection.length === 0) {
        kindo.util.alert('请至少选择一条标准诊断', undefined, 'warning')
        return
      }

      if (this.standardSelection.length > 1 && this.hosSelection.length > 1) {
        kindo.util.alert('不能多条对多条数据进行关联', undefined, 'warning')
        return
      }

      let param = {
        hosIds: this.hosSelection.map(item => item.id).toString(),
        standardIds: this.standardSelection.map(item => item.code).toString()
      }
    ...
    standardOnSelect(selection) {
      this.standardSelection = selection
    },
