    ...
    <el-row>
      <el-col :span="13">
        <el-form inline :model="hosForm" :rules="rules" ref="hosModel" onsubmit="return false;" @keyup.enter.native="getHos">
          <el-form-item prop="name">
            <el-tooltip effect="dark" content="请输入医院操作编码或名称" placement="top">
              <el-input v-model="hosForm.name" placeholder="医院操作..." style="width:185px;"></el-input>
            </el-tooltip>
          </el-form-item>
          <el-form-item>
            <el-button icon="search" @click="getHos">查 询</el-button>
            <!-- <el-button icon="upload" v-if="kindo.cache.get(kindo.constantUSER_INFO).loginNo === 'kindo'" @click="() => { this.uploadVisible = true }">上传医院编码</el-button> -->
        ...
        <kindo-table :url="hosApi" ref="hos" :queryParam="hosForm" :selection-change="hosOnSelect">
          ...
          <el-table-column prop="code" label="医院操作编码" width="145" sortable="custom" show-overflow-tooltip>
            <template slot-scope="scope">
              <span>{{ scope.row.code }}</span>
              <span v-if="scope.row.appendCode" style="color: firebrick">( {{ scope.row.appendCode }} )</span>
            </template>
          </el-table-column>
          <el-table-column prop="name" label="医院操作名称" min-width="125" sortable="custom" show-overflow-tooltip> </el-table-column>
      ...
      <el-col :span="10" :offset="1">
        <el-form inline :model="standardForm" :rules="rules" ref="standardModel" onsubmit="return false;" @keyup.enter.native="getStandard">
          <el-form-item prop="name">
            <el-tooltip effect="dark" content="请输入标准操作编码或名称" placement="top">
              <el-input v-model="standardForm.name" placeholder="标准操作..." style="width:185px;"></el-input>
            </el-tooltip>
          </el-form-item>
          <el-form-item>
        ...
        <kindo-table :url="standardApi" ref="standard" :queryParam="standardCondition">
          <el-table-column prop="code" label="标准操作编码" width="130" sortable="custom" show-overflow-tooltip>
            <template slot-scope="scope">
              <span>{{ scope.row.code }}</span>
              <span v-if="scope.row.appendCode" style="color: firebrick">( {{ scope.row.appendCode }} )</span>
            </template>
          </el-table-column>
          <el-table-column prop="name" label="标准操作名称" min-width="175" sortable="custom" show-overflow-tooltip inline-template>
  ...
  methods: {
    ...
    submitForRecommend(row) {
      if (this.hosSelection.length === 0) {
        kindo.util.alert('请至少选择一条医院操作', undefined, 'warning')
        return
      }
