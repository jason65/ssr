  ...
  <div>
      ...
      <el-form inline :model="form" :rules="rules" ref="Model" onsubmit="return false;" @keyup.enter.native="get">
        <el-form-item prop="name">
          <el-tooltip effect="dark" content="请输入医院操作编码或名称" placement="top">
            <el-input v-model="form.name" placeholder="医院操作..." style="width:185px;"></el-input>
          </el-tooltip>
        </el-form-item>
        <el-form-item prop="standardName">
          <el-tooltip effect="dark" content="请输入标准操作编码或名称" placement="top">
            <el-input v-model="form.standardName" placeholder="标准操作..." style="width:185px;"></el-input>
          </el-tooltip>
        </el-form-item>
        <el-form-item>
    ...
    <kindo-box>
      <kindo-table :url="tableApi" ref="table" :queryParam="form">
        <el-table-column prop="hosCode" label="医院操作编码" width="130" align="left" sortable="custom" show-overflow-tooltip> </el-table-column>
        <el-table-column prop="name" label="医院操作名称" min-width="220" align="left" sortable="custom" show-overflow-tooltip></el-table-column>
        <el-table-column prop="standardCode" label="标准操作编码" width="130" align="left" sortable="custom" show-overflow-tooltip>
          <template slot-scope="scope">
            <span>{{ scope.row.standardCode }}</span>
            <span v-if="scope.row.appendCode" style="color: firebrick">( {{ scope.row.appendCode }} )</span>
          </template>
        </el-table-column>
        <el-table-column prop="standardName" label="标准操作名称" min-width="220" align="left" sortable="custom" show-overflow-tooltip></el-table-column>
        <el-table-column prop="djbs" label="对接标识" width="120" align="left" sortable="custom">
          <template slot-scope="scope">
        ...
        <el-table-column label="操作" width="140" align="center" show-overflow-tooltip>
          <template slot-scope="scope">
            ...
            <!-- 天津暂时去掉审核流程 -->
            <!--<el-button icon="d-arrow-right" @click="resolve(scope.row)"> 通 过 </el-button>-->
          </template>
        </el-table-column>
      </kindo-table>
    </kindo-box>
  ...
  data() {
    return {
      ...
      rules: {
        ...
        standardName: [{ min: 0, max: 64, message: '请最多输入64个数字字母或汉字', trigger: 'blur' }]
      }
    }
  },
  ...
  created() {
    this.$http.get(config.api.getDeptName).then(res => {
      if (res && res.data && (res.data instanceof Array)) {
        this.source.deptName = res.data.map(item => {
