  ...
  data() {
    return {
      ...
      deptCapacityOps: {
        tooltip: {
          formatter: obj => {
            if (obj.data.hasOwnProperty('siblingName')) {
              return (
                `<div style="border-bottom: 1px solid rgba(255,255,255,.3); font-size: 18px;padding-bottom:5px;margin-bottom: 5px">
                ${obj.name}
              </div> 
                ${obj.data.valueName}： ${obj.data.value}
                <br>
                 ${obj.data.siblingName}： ${obj.data.siblingValue}`
              )
            } else {
    ...
    queryDetp() {
      this.$http.get(config.api.hoscapacity, { params: this.searchForm }).then(res => {
        ...
        if (res.data.rows && res.data.rows.length) {
          // 这里做排序
          sortArr = res.data.rows.sort((itemA, itemB) => {
            return itemB.coordX - itemA.coordX
          })
          sortArr.forEach((o, i) => {
            // tempArr.push({ name: o.name, value: [o.coordX, o.coordY] })
            xAsisArr.push(o.name)
            tempArr1.push({ value: o.coordY, valueName: 'CMI', siblingName: 'DRG组数', siblingValue: o.coordX })
            tempArr2.push({ value: o.coordX, valueName: 'DRG组数', siblingName: 'CMI', siblingValue: o.coordY })
          })
          let tempObj = { show: true, end: 100 }
          // 标准为25个
          if (xAsisArr.length < 15) {
            tempObj.show = false
            tempObj.end = 100
          } else {
            tempObj.show = true
            tempObj.end = parseInt(15 / xAsisArr.length * 100)
          }
          this.$refs.deptCapacityRef.mergeOptions({
            series: [
              { data: tempArr1, markLine: { data: [{ yAxis: kindo.util.formatNum(res.data.averageX), name: '平均值' }] } },
              { data: tempArr2, markLine: { data: [{ yAxis: kindo.util.formatNum(res.data.averageY), name: '平均值' }] } }
            ],
            xAxis: { data: xAsisArr },
            dataZoom: [tempObj]
          })
